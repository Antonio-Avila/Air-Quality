...
# NEED TO FIND A BETTER WAY TO SPLIT UP THE DIFFERENT WAVES. LOOK INTO HOW TO PASS ON THE WAVE VALUE. GETTING A MEMORY ERROR WHEN ATTEMPTING TO JOIN DAYS BEFORE DATA WITH THE FILTERED POLLUTION DATA.
test_county = "Dallas"
test_data <-  covid_cases %>% filter(county == test_county) 
...
# test_data %>% filter(date %in% days_before$date) %>% left_join(days_before) %>% merge(days_before)

# Median pm leading up to spike in cases/deaths
daily_avg_pm2.5_county %>% filter(county == test_county, date_local %in% days_before$date) %>% 
  ...
  group_by(wave) %>% summarize(date.ish = mean(date_local), avg_pm = median(pm2.5_avg))
...
daily_avg_pm2.5_county[daily_avg_pm2.5_county$county == test_county & daily_avg_pm2.5_county$date_local %in% days_before$date, ][(1+i*n):(n+n*i), ] %>%
  select(pm2.5_avg) %>% 
  t.test(mu = 15, alternative = "greater")
```
...
p_cases <- covid_cases %>%  filter(county == test_county, date >= start_date, date <= end_date) %>%  
  ...
  ggtitle(paste("Daily COVID related Cases per 100,000 in", test_county, "County")) +
  theme(legend.position = "none")
# ggplotly(p, dynamicTicks = TRUE) %>% layout(hovermode = "x")
...
p_fatal <- covid_fatal %>%  filter(county == test_county, date >= start_date, date <= end_date) %>%  
  ...
  geom_line(aes(color = county)) +
  ggtitle(paste("Daily COVID related Fatalities per 100,000 in", test_county, "County")) +
  theme(legend.position = "none")
# ggplotly(p, dynamicTicks = TRUE) %>% layout(hovermode = "x")
