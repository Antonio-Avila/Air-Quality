...
covid_hospital <- covid_hospital %>% group_by(county) %>% 
  ...
  mutate(scaled_conf_hosp_7day = total_conf_hosp_7day / Population * 100000, scaled_conf_and_sus_hosp_7day = total_conf_and_sus_hosp_7day / Population * 100000)
...
p_hosp <- covid_hospital %>% filter(county == test_county, date >= start_date, date <= end_date) %>% 
  ggplot(aes(date, scaled_conf_hosp_7day)) + 
    ...
    ggtitle(paste("Daily COVID related Hospitalization per 100,000 in", test_county, "County")) +
...
data = covid_cases %>% filter(County == test_county) %>% select(Date, new_cases) %>% ungroup()
Acf(data$new_cases, lag.max = 200)
Pacf(data$new_cases, lag.max = 150)

model = auto.arima(data$new_cases)
summary(model)
...
```{r creating function to find max vals}

find_max <- function(data){
    x = c()
    for (i in 1:3) {
      x <- append(x, max(data[(222*(i - 1)):(i*222)]))
    }
  return(x)
}

# Example of how to apply and plot local maximums

# maxim = find_max(data$new_cases)
# maxim
# data["peak"] = data$new_cases %in% maxim
# 
# data %>% ungroup() %>% 
#   ggplot(aes(Date, new_cases)) + 
#     geom_line() +
#     geom_point(data = . %>% filter(new_cases %in% maxim), aes(y = new_cases), color = "red")
...
covid_cases %>% filter(County == "Harris") %>% 
  ggplot(aes(Date, cases_7day_avg)) + 
    geom_line() + 
    geom_smooth(method = forecast::auto.arima)
...
p <- covid_fatal %>% filter(County == test_county) %>%  
  ...
  geom_line(color = "#32213A") +
  xlab("Date") + ylab("New Cases") + ggtitle("New COVID Fatalities per 100,000 People")
ggplotly(p)
