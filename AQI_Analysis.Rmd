...
data = covid_cases %>% filter(County == test_county) %>% select(Date, new_cases)
...
```{r creating function to calculate }


find_max <- function(data){
    x = c()
    for (i in 1:3){
      x <- append(x, (which.max(data[(222*(i-1)):(i*222)]) + 222 * (i-1)))
    }
  return(x)
}
