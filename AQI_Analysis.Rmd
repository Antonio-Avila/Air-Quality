...
# Sys.sleep(120)
# start_time = Sys.time()
# pm2.5_texas_22 <- aqs_sampledata_by_state(parameter = "88101",
#                                            bdate = as.Date("20210901",
#                                                            format="%Y%m%d"
#                                                            ),
#                                            edate = as.Date("20220501",
...
# pm2.5_texas <- bind_rows(pm2.5_texas, pm2.5_texas_2022)
...
test_data <- covid_hospital %>% filter(county == "Harris")
maxim <- test_data[findpeaks(test_data$total_conf_hosp_7day, ndowns = 2, npeaks = 3, nups = 2)[, 2], ]
p <- test_data %>%
...
daily_avg_pm2.5_county %>% filter(county == test_county) %>% 
  ...
  t_test(response = pm2.5_avg, mu = 15)


daily_avg_pm2.5_county %>% filter(county == test_county, date_local %in% days_before$date) %>% 
  left_join(waves, by = c("date_local" = "date")) %>% 
  group_by(wave) %>% 
...
days_high_pm <- daily_avg_pm2.5_county %>% mutate(year = year(date_local), month = month(date_local)) %>% 
  group_by(county, year, month) %>% 
  summarize(n_high_pm = sum(pm2.5_avg > 15, na.rm = TRUE)) %>% 
  ungroup() %>% 
...
p <- covid_hospital %>% filter(county == "Harris", date <= "2022-01-01") %>%  ggplot(aes(date, scaled_conf_hosp_7day)) + geom_line()
covid_fatal %>% filter(county == "Harris", date >= "2020-07-31") %>% ggplot(aes(date, fatal_7day_avg)) + geom_line()
ggplotly(p, dynamicTicks = TRUE) %>% layout(hovermode = "x")
```
...
p_pm <- pm_7day %>% 
  filter(county %in% test_county, date_local >= "2020-03-01", date_local <= "2021-12-31") %>%
  ggplot(aes(date_local, pm_7day)) + geom_line(aes(color = county)) +
  geom_hline(yintercept = 15, color = "red") + theme(legend.position = "none") +
