...
p <- daily_avg_pm2.5_county %>% mutate(year = year(date_local), month = month(date_local)) %>% 
  group_by(county, year, month) %>% 
  summarize(n_high_pm = sum(pm2.5_avg > 15)) %>% 
  ungroup() %>% 
  mutate(date = ymd(paste(year, month, "01"))) %>% 
  filter(county == "Harris") %>%
  ggplot(aes(date, n_high_pm)) + geom_line()
ggplotly(p)
