...
# Sys.sleep(120)
# start_time = Sys.time()
# pm2.5_texas_17 <- aqs_sampledata_by_state(parameter = "88101",
#                                            bdate = as.Date("20170101",
#                                                            format="%Y%m%d"
#                                                            ),
#                                            edate = as.Date("20170901",
#                                                           format = "%Y%m%d"),
#                                            stateFIPS = "48"
#                                           )
# write_csv(pm2.5_texas_17, "pm2.5_texas_2017.csv")
# Sys.time() - start_time
# beep()
```
...
# Import COVID Hospitalization Data and subset the data that is needed. Also renaming columns to be more concise.
...
NEED TO IMPROVE FIND MAX FUNCITON. I WOULD ASSUME THE DATE OF THE MAX COVID CASES WOULD BE BEFORE OR COINCIDE THE MAX OF COVID FATALITIES, NOT BE SEVERAL MONTHS BEFORE AS IT IS IN HARRIS COUNTY. NEED TO CHECK WITH PLOTS. 

MAY NEED TO FIT SMOOTHER CURVES OR FUNCTIONS TO THE DATA AND USE THE CURVE AND A MORE TRADITIONAL ALGO TO FIND THE MAX'S. GOES BACK TO THE IDEA OF MODELING. 
...
find_max <- function(data, variable){
  
    variable <- as.character(variable)
    maxim <-  tibble(matrix(nrow = 0, ncol = 2))
    colnames(maxim) <- c("Date", variable)
    x <- split(data, rep(1:3, length.out = nrow(data), each = ceiling(nrow(data) / 3)))
    
    for (i in 1:3) {
      maxim <- rbind(maxim, x[[i]][which.max(x[[i]][[variable]]),])
      # maxim[i, "wave"] = i
    }
    maxim["wave"] = c(1, 2, 3)
  return(maxim)
...
find_max_hosp <- function(data = covid_hospital, county = "Harris", variable = "total_conf_hosp_7day"){
  data_filtered <- data[data$county == county, ]
  maxim <- data_filtered[findpeaks(data_filtered[variable][[1]], ndowns = 3, nups = 3)[, 2], ]
  maxim["wave"] = 1:nrow(maxim)
  return(maxim)
}
```


```{r testing find max function}

# Covid and Fatality Data
# test_county = "Dallas"
# test_data <-  covid_cases %>% filter(county == test_county) 
# 
# maxim = find_max(test_data, 'scaled_new_cases')
# 
# test_data %>% ungroup() %>%
#   ggplot(aes(date, scaled_new_cases)) +
#     geom_line() +
#     geom_point(data = . %>% filter(date %in% maxim$date), aes(y = scaled_new_cases), color = "red")
...
test_county == "Harris"
maxim <- find_max_hosp(covid_hospital, test_county, "total_conf_hosp_7day")
maxim

p <- covid_hospital %>% filter(county == test_county) %>% 
   ggplot(aes(date, total_conf_hosp_7day)) +
   geom_line() +
   geom_point(data = . %>% filter(county == "Harris", date %in% maxim$date), aes(y = total_conf_hosp_7day), color = "red")
...
daily_max_counties <- read_csv("ozone_daily_max_counties.csv")
```
...
p_ill <- daily_ill %>% filter(county == "Cook", date_local >= "2017-09-01") %>% 
  ...
  geom_line(data =  daily_avg_pm2.5_county %>% filter(county == "Harris", date_local >= "2017-01-01", date_local <= "2018-01-01"), 
            aes(y = pm2.5_avg), color = "blue")
ggplotly(p_ill, dynamicTicks = T) %>% layout(hovermode = "x")
  
  
p_harr <- daily_avg_pm2.5_county %>% 
...
#   summarise(pm2.5_avg = max(site_avg, na.rm = TRUE)) %>% ungroup()
# daily_avg_pm2.5_county[is.infinite(daily_avg_pm2.5_county$pm2.5_avg), ]$pm2.5_avg <- NA
# # 
# write_csv(daily_avg_pm2.5_county, "pm2.5_daily_avg_counties.csv")
daily_avg_pm2.5_county <- read_csv("pm2.5_daily_avg_counties.csv")
daily_avg_pm2.5_county <- daily_avg_pm2.5_county %>% group_by(county) %>% 
...
# Harris County
test_county = "Harris"
p <- daily_avg_pm2.5_county %>% filter(county == test_county) %>% 
...
test_dat <- daily_avg_pm2.5_county %>% 
...
In this step, I am calculating the number of new cases and fatalities per day as well as the rolling average for each. Moreover, I am including a scaled version of each metric to aptly compare between different locations by converting to cases/fatalities per 100,000 residents. The hospitalization data is sourced from a facility level data for hospital utilization aggregated on a weekly basis. N/A's indicate a no response from the hospital while suppression was applied to protect the patients identity when the number of patients was less than 4. In order to aggregate the data county to compare to the other datasets, I will be replacing the suppressed values with 2, since the values represent 1, 2, or 3 patients. When there are 0 and 4 patients hospitalized, the value is recorded.

```{r number of new covid cases}
...
covid_hospital <- covid_hospital %>% group_by(county) %>% 
  left_join(texas_pop, by = c("county" = "County")) %>% 
  mutate(scaled_conf_hosp_7day = total_conf_hosp_7day / Population * 100000, scaled_conf_and_sus_hosp_7day = total_conf_and_sus_hosp_7day / Population * 100000) %>% ungroup()
...
```{r check to see if there is a spike in pollution a week before the spike in covid fatalities}


test_county = "Harris"
test_data <-  covid_fatal %>% filter(county == test_county) 

maxim = find_max(test_data, 'scaled_new_fatal')
n = 14
days_before <- n_days_before(maxim, n)
days_after <- n_days_after(maxim, n)

# Various PM stats of days leading up to spike in cases/deaths
daily_avg_pm2.5_county %>% filter(county == test_county) %>% 
  right_join(days_before, by = c('date_local' = 'date')) %>% 
  group_by(wave) %>% summarize(start_date = min(date_local), end_date = max(date_local),
                                avg_pm = mean(pm2.5_avg, na.rm = TRUE), med_pm = median(pm2.5_avg, na.rm = TRUE), 
                                min_pm = min(pm2.5_avg, na.rm = TRUE), max_pm = max(pm2.5_avg, na.rm = TRUE)) %>% 
  right_join(maxim, by = "wave")


# Various PM stats of days leading up to spike in cases/deaths
daily_avg_pm2.5_county %>% filter(county == test_county) %>% 
  right_join(days_after, by = c('date_local' = 'date')) %>% 
  group_by(wave) %>% summarize(start_date = min(date_local), end_date = max(date_local),
                                avg_pm = mean(pm2.5_avg, na.rm = TRUE), med_pm = median(pm2.5_avg, na.rm = TRUE), 
                                min_pm = min(pm2.5_avg, na.rm = TRUE), max_pm = max(pm2.5_avg, na.rm = TRUE)) %>% 
  right_join(maxim, by = "wave")
...
# Yearly avg, median, min, max of PM
daily_avg_pm2.5_county %>% filter(county == test_county, !(date_local %in% days_before$date)) %>% 
  group_by(county) %>% 
  summarize(avg_pm = mean(pm2.5_avg, na.rm = TRUE), med_pm = median(pm2.5_avg, na.rm = TRUE), 
            min_pm = min(pm2.5_avg, na.rm = TRUE), max_pm = max(pm2.5_avg, na.rm = TRUE))  %>% 
  right_join(maxim, by = "wave")
...
# Annual pm avg
# p <- daily_avg_pm2.5_county %>% mutate(year = year(date_local)) %>% group_by(county, year) %>%
#   summarize(yearly_pm = mean(pm2.5_avg, na.rm = TRUE)) %>%  
#   ggplot(aes(year, yearly_pm, color = county)) + geom_line() + geom_hline(yintercept = 12, color = "red")
# ggplotly(p, dynamicTicks = TRUE)


daily_avg_pm2.5_county %>% filter(county == test_county) %>% 
  left_join(days_before, by = c("date_local" = "date")) %>% 
  filter(!is.na(wave), wave == 1) %>% 
  t_test(response = pm2.5_avg, mu = 12, alternative = "greater")


daily_avg_pm2.5_county %>% filter(county == test_county, date_local %in% days_before$date) %>% 
  left_join(wave, by = c("date_local" = "date")) %>% 
  group_by(wave) %>% 
  summarize(t_score = observe(response = pm2.5_avg, stat = "mean"))


daily_avg_pm2.5_county %>% filter(county == test_county) %>% 
  right_join(days_after, by = c('date_local' = 'date')) %>% 
  group_by(wave) %>% arrange(desc())
...
p_cases <- covid_cases %>%  filter(county == test_county, date >= start_date, date <= end_date) %>%  
  ggplot(aes(x = date, y = cases_7day_avg)) + 
  geom_line(aes(color = county)) +
  ggtitle(paste("7-day Avg og COVID related Cases per 100,000 in", test_county, "County")) +
  theme(legend.position = "none")
# ggplotly(p, dynamicTicks = TRUE) %>% layout(hovermode = "x")
...
p_fatal <- covid_fatal %>%  filter(county == test_county, date >= start_date, date <= end_date) %>%  
  ggplot(aes(x = date, y = fatal_7day_avg)) + 
  geom_line(aes(color = county)) +
  ggtitle(paste("7-day Avg og COVID related Fatalities per 100,000 in", test_county, "County")) +
  theme(legend.position = "none")
# ggplotly(p, dynamicTicks = TRUE) %>% layout(hovermode = "x")
...
```{r messing with arima on covid cases}

test_county = "Harris"
covid_cases %>% filter(county == test_county, date >= "2020-03-06", date <= "2021-12-31") %>% 
...
data = covid_cases %>% filter(county == test_county) %>% select(date, cases_7day_avg) 
Acf(data$cases_7day_avg, lag.max = 400)
Pacf(data$cases_7day_avg, lag.max = 400)

model = auto.arima(data$cases_7day_avg[1:length(data$cases_7day_avg)-1])
summary(model)

data[8:nrow(data)-1, ] %>% 
  ggplot(aes(date)) +
  geom_line(aes(y = cases_7day_avg)) +
  geom_line(aes(y = model$fitted), color = "red")

preds = forecast(model, 10)
autoplot(preds)


Acf(model$residuals, 200)
Pacf(model$residuals, 200)
...
p_pm <- filter(daily_avg_pm2.5_county, county == test_county, date_local >= "2020-03-06", date_local <= "2021-12-31") %>% 
  ggplot(aes(x = date_local, y = pm2.5_avg)) +
...
monthly_pm <- daily_avg_pm2.5_county %>% mutate(year = year(date_local), month = month(date_local)) %>% 
...
test_data <- covid_cases %>% filter(county == "Harris") %>% select(date, county, cases_7day_avg) %>% 
  left_join(daily_avg_pm2.5_county %>% filter(county == "Harris"), 
...
```{r plot covid hospitalization data}

p <- covid_hospital %>% filter(county == "Harris") %>%  ggplot(aes(date, scaled_conf_hosp_7day)) + geom_line()
ggplotly(p, dynamicTicks = TRUE) %>% layout(hovermode = "x")
...
p <- covid_hospital %>% filter(county == test_county) %>% 
   ggplot(aes(date, total_conf_hosp_7day)) +
   geom_line() +
   geom_point(data = . %>% filter(county == test_county, date %in% maxim$date), aes(y = total_conf_hosp_7day), color = "red")
ggplotly(p, dynamicTicks = TRUE) %>% layout(hovermode = "x")

```
 
 
The average of the PM2.5 levels are not what I was expecting. In general, a rise in covid hospitalizations do not tend to affect the PM2.5 levels. More often than not, it actually increases or has no affect on the PM2.5 values. I was expecting to observe 2 things: to see elevated levels prior to the peak covid hospitalizations and to see a decrease in PM2.5 after the peak. The former does seem to be occur somewhat frequently but the latter does not. In fact, the opposite happens. PM2.5 usually increases over the week following peak covid hospitalizations. 
...
days_before <- n_days_before(maxim, 7)
days_after <- n_days_after(maxim, 7)

daily_avg_pm2.5_county %>% filter(county == test_county) %>% 
  right_join(days_before, by = c("date_local" = "date")) %>% 
  group_by(wave) %>% 
  summarise(mean_pm = mean(pm2.5_avg, na.rm = TRUE))
