...
covid_hospital %>% left_join(county_codes, by = "fips") %>% View()
...
# Calculate the number of hospitalizations in each county. 
...
# Plotting the total number of cases and fatalities per day in Harris and Dallas County (containing the 2 largest cities in Texas, Houston and Dallas)
covid_cases %>% filter(County %in% c("Dallas", 'Harris')) %>%  ggplot(aes(x = Date, y = Cases)) + 
  geom_line(aes(color = County)) 

# Plotting the new cases in the above counties
p <- covid_cases %>% filter(County %in% c("Dallas", 'Harris')) %>%  ggplot(aes(x = Date, y = new_cases)) + 
  geom_line(aes(color = County)) +
  ggtitle("Raw New Cases")
ggplotly(p, dynamicTicks = TRUE) %>% layout(hovermode = "x")
...
p <- covid_cases %>% filter(County == "Harris") %>% 
  ggplot(aes(x = Date, y = new_cases)) + 
    geom_line() +
    geom_line(aes(y = cases_7day_avg), color = "red") 
ggplotly(p, dynamicTicks = TRUE) %>% layout(hovermode = "x")
...
p <- covid_cases %>%  filter(County %in% c("Dallas", 'Harris')) %>%  ggplot(aes(x = Date, y = scaled_new_cases)) + 
  geom_line(aes(color = County)) +
  ggtitle("New Cases per 100,000")
ggplotly(p, dynamicTicks = TRUE) %>% layout(hovermode = "x")


# Pot the number of new fatalities per 100,000 people
p <- covid_fatal %>%  filter(County %in% c("Dallas", 'Harris', 'Travis')) %>%  ggplot(aes(x = Date, y = scaled_new_fatal)) + 
  geom_line(aes(color = County)) +
  ggtitle("Scaled New Fatalities")
ggplotly(p, dynamicTicks = TRUE) %>% layout(hovermode = "x")
```
...
```{r pm vs covid on same plot}

# Plot covid fatalities with pm2.5 
test_county <- "Harris"
p <- covid_fatal %>% filter(County == test_county, Date >= "2020-03-06") %>% 
  filter(County == test_county, Date <= "2021-09-01") %>% 
  inner_join(daily_avg_pm2.5_county, by = c("Date" = "date_local", "County" = "county")) %>% 
  ggplot(aes(x = Date)) + 
    geom_line(aes(y = new_fatal), color = "#32213A") +
    geom_line(aes(y = pm2.5_avg), color = "#2176AE") +
    geom_hline(yintercept = 15, color = "red") +
    ggtitle("Daily Covid Fatalities per 100,000") +
    xlab("Date") + ylab("Fatalities")
ggplotly(p, dynamicTicks = TRUE) %>% layout(hovermode = "x")
...
pm_7day <- daily_avg_pm2.5_county %>% group_by(county) %>% 
