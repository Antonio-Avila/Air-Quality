...
# Excldue the non-missing values without a qualifier, the vast majority of samples
...
p <- pm2.5_texas %>%  filter(county == "Harris") %>% 
  ggplot(aes(x = ymd(date_local) + hms(time_local), y = sample_measurement)) + 
      geom_line(aes(color = site_number))
ggplotly(p, dynamicTicks = TRUE)
...
pm2.5_texas %>% filter(sample_measurement > 55) %>% arrange(desc(sample_measurement)) %>% View()

```


```{r}
pm2.5_texas %>% filter(qualifier == "X - Filter Temperature Difference or Average out of Spec.") %>% count(sample_frequency)
```
...
p <- daily_avg_pm2.5_county %>% filter(county == "Harris") %>% 
        ggplot(aes(x = date_local, y = county_avg)) + 
...
data <- daily_avg_pm2.5_county %>% filter(county == "Harris") %>% select(county_avg) %>% drop_na() 
