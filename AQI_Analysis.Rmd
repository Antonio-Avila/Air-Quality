...
p <- covid_cases %>% filter(County %in% c("Travis", 'Harris')) %>%  ggplot(aes(x = Date, y = new_cases)) + 
  geom_line(aes(color = County)) +
...
p <- covid_cases %>%  filter(County %in% c("Dallas", 'Harris', 'Travis')) %>%  ggplot(aes(x = Date, y = scaled_new_cases)) + 
  geom_line(aes(color = County)) +
  ggtitle("Scaled New Cases")
ggplotly(p, dynamicTicks = TRUE) %>% layout(hovermode = "x")
...
p_cov <- covid_fatal %>% filter(County == "Harris", Date >= "2020-03-06") %>% 
...
p_pm <- filter(daily_avg_pm2.5_county, county == "Harris", date_local >= "2020-03-06") %>% 
  ggplot(aes(x = date_local)) +
      geom_line(aes(y = pm2.5_avg), color = "blue")   +
      xlab("Date") + ylab("PM2.5 Daily Avg")  
  
grid.arrange(p_cov, p_pm)
...
covid_cases %>% filter(County == "Harris", Date >= "2020-03-06") %>% 
  filter(County == "Harris", Date <= "2021-09-01") %>% 
  inner_join(daily_avg_pm2.5_county, by = c("Date" = "date_local", "County" = "county")) %>% 
  ggplot(aes(x = Date)) + 
...
daily_avg_pm2.5_county %>% mutate(year = year(date_local), month = month(date_local)) %>% 
  group_by(year, month) %>% 
