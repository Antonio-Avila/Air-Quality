...
covid_cases %>% filter(County == "Harris", Date > "2020-03-06") %>%  ggplot(aes(x = Date, y = Cases)) + 
  geom_line() +
...
diff_cases <- data.frame(diff_cases = diff(covid_cases[covid_cases$County == "Harris",]$Cases))
diff_cases["date"] <- covid_cases %>% filter(County == "Harris", Date > "2020-03-06") %>% select(Date)
diff_cases$date

p <- diff_cases %>% ggplot(aes(date, diff_cases)) + geom_line()
ggplotly(p, dynamicTicks = TRUE) %>% layout(hovermode = "x")

covid_cases %>% filter(County == "Harris", Date >= "2021-06-29", Date <= "2021-07-01")





# Difference matches daily cases reported by Harris County, 7 day average does as well in this case. Create a function calculating 7day avg
diff_cases %>% filter(date >= "2021-01-09", date <= "2021-01-15") %>% summarize(avg = mean(diff_cases))
```
