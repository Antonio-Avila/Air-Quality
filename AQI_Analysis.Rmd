...
prev_7day_avg <- function(data){
  avg = matrix(nrow = length(test), ncol = 1)
  avg[1:6] <- NA
...
daily_avg_pm2.5_county %>% group_by(county) %>% summarise(avg_7day = prev_7day_avg(pm2.5_avg))
...
daily_avg_pm2.5_county <- daily_avg_pm2.5_county %>% group_by(county) %>% 
  mutate(pm_7day = rollmeanr(pm2.5_avg, 7, fill = NA)) %>% ungroup()
```
...
p_cases <- covid_cases %>%  filter(County == test_county, Date >= start_date, Date <= end_date) %>%  
  ...
  ggtitle("Daily COVID related Cases per 100,000") +
  theme(legend.position = "none")
# ggplotly(p, dynamicTicks = TRUE) %>% layout(hovermode = "x")
...
p_fatal <- covid_fatal %>%  filter(County == test_county, Date >= start_date, Date <= end_date) %>%  
  ...
  geom_line(aes(color = County)) +
  ggtitle("Daily COVID related Fatalities per 100,000") +
  theme(legend.position = "none")
# ggplotly(p, dynamicTicks = TRUE) %>% layout(hovermode = "x")
...
p_hosp <- covid_hospital %>% filter(county == test_county, date >= start_date, date <= end_date) %>% 
  ggplot(aes(date, scaled_conf_hosp_7day)) + 
    ...
    ggtitle("Daily COVID related Hospitalization per 100,000") +
...
p_cases <- covid_cases %>% filter(County == test_county, Date >= "2020-03-06", Date <= "2021-12-31") %>% 
  ggplot(aes(x = Date)) + 
    geom_line(aes(y = scaled_new_cases), color = "#424874") +
...
p_fatal <- covid_fatal %>% filter(County == test_county, Date >= "2020-03-06", Date <= "2021-12-31") %>% 
  ggplot(aes(x = Date)) + 
    geom_line(aes(y = scaled_new_fatal), color = "red") +
...
p_pm <- filter(daily_avg_pm2.5_county, county == test_county, date_local >= "2020-03-06", date_local <= "2021-12-31") %>% 
  ggplot(aes(x = date_local)) +
      geom_line(aes(y = pm2.5_avg), color = "#5E747F") +
      geom_hline(yintercept = 12, color = "red") +
...
grid.arrange(p1, p_cases, p_fatal_month)
```
...
test_county = "Dallas"
p1 <- pm_7day %>% 
  filter(county == test_county, date_local >= "2020-03-01", date_local <= "2021-12-31") %>%
...
# ggplotly(p2) %>% layout(hovermode = "x")
