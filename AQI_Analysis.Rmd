...
# Import COVID data
covid_cases <- read_excel("texas_covid_cases.xlsx")



path <- "texas_covid_cases.xlsx"
covid_cases <- path %>% excel_sheets() %>% set_names() %>% map_dfr(read_excel, path = path, skip = 2, col_names = TRUE)
covid_cases <- slice(covid_cases, 1:(n() - 6)) %>% tail()

as.Date(as.Date("2020-01-01"):as.Date("2020-12-31"))
...
covid_cases
covid_fatal <- read_csv("texas_covid_fatalities.csv")
```
