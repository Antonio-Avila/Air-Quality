...
```{r}
p <- harris_county %>% ggplot(aes(x = date_local)) +
  geom_line(aes(y = county_max, color = "blue")) +
  geom_line(aes(y = cumsum(c(78, pred$fitted)), color = "red")) +
  scale_color_manual(name="Values",labels=c("Measured Values", "Predictions"), values=c("dodgerblue4", "firebrick4"))
ggplotly(p, dynamicTicks = TRUE) %>% layout(hovermode = "x") %>% rangeslider()
...
Time series analys relies around around the concept of the ideal scenario involving stationary data. Many of the results 

```{r percent change}

ozone_perc_change <- data.frame(date_local = harris_county$date_local, perc_change = Delt(harris_county$county_max))
ozone_perc_change <- ozone_perc_change %>% dplyr::rename(perc_change = Delt.1.arithmetic)
ggplot(ozone_perc_change, aes(perc_change)) + geom_histogram()

p <- ozone_perc_change %>% ggplot(aes(date_local, perc_change)) +
      geom_line(aes(color = "Ozone Percent Increase")) +
      geom_smooth(aes(color = "Smoothing funciton")) +
      scale_color_manual(name = "legend", values = c("#69b3a2", "#00A9FF")) +
      xlab("Date") +
      ylab("Percent Increase") +
      ggtitle("Ozone Percent Increase from Previous Day")
ggplotly(p, dynamicTicks = TRUE) %>%  layout(hovermode = "x") 



t.test(ozone_perc_change$perc_change)
adf.test(ozone_perc_change$perc_change[-1], k = 365)
Box.test(ozone_perc_change$perc_change[-1], type = "Ljung", lag = 365)

par(mfcol = c(2,1))
Acf(ozone_perc_change$perc_change, lag.max = 720)
Pacf(ozone_perc_change$perc_change, lag.max = 720)
par(mfcol = c(1,1))
