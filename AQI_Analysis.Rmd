...
#Need to clean county names
texas_pop$County %>% str_extract_all("County")
...
covid_cases <- covid_cases %>% group_by(County) %>% 
  ...
  mutate(cases_7day_avg = round(rollmeanr(new_cases, 7, fill = NA))) %>% ungroup() %>% as_tibble()


covid_fatal <- covid_fatal %>% group_by(County) %>% 
  mutate(new_fatal = c(0, diff(Fatalities))) %>% group_by(County) %>% 
  mutate(fatal_7day_avg = round(rollmeanr(new_fatal, 7, fill = NA))) %>% ungroup() %>% as_tibble()
...
covid_pm <- covid_cases %>% filter(County %in% unique(pm2.5_texas$county)) 
daily_avg_pm2.5_county %>% filter(county == "Harris") %>% tail()

daily_avg_pm2.5_county %>% mutate(year = year(date_local)) %>% group_by(county, year) %>% summarize(high_pm = sum(pm2.5_avg > 12), high_pm_perc = high_pm / length(pm2.5_avg))
