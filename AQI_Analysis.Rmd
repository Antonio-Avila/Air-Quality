...
```{r plot cases and fatalities per 100000}
...
test_county = "Harris"
p_cases <- covid_cases %>% filter(County == test_county, Date >= "2020-03-06", Date <= "2021-09-01") %>% 
  ggplot(aes(x = Date)) + 
    geom_line(aes(y = scaled_new_cases), color = "#424874") +
    ggtitle(paste("Daily Covid Cases per 100,000 in", test_county, "County")) +
    xlab("Date") + ylab("Cases")

p_fatal <- covid_fatal %>% filter(County == test_county, Date >= "2020-03-06", Date <= "2021-09-01") %>% 
  ggplot(aes(x = Date)) + 
...
p_pm <- filter(daily_avg_pm2.5_county, county == test_county, date_local >= "2020-03-06") %>% 
...
p <- covid_fatal %>% filter(County == test_county, Date >= "2020-03-06") %>% 
  ...
  ggplot(aes(x = Date)) + 
    geom_line(aes(y = new_fatal), color = "red") +
    geom_line(aes(y = pm2.5_avg), color = "blue", alpha = 0.3) +
...
# plot number of daily covid cases with pm2.5
p <- covid_cases %>% filter(County == "Harris", Date >= "2020-03-06") %>% 
  filter(County == "Harris", Date <= "2021-09-01") %>% 
  inner_join(daily_avg_pm2.5_county, by = c("Date" = "date_local", "County" = "county")) %>% 
  ggplot(aes(x = Date)) + 
    geom_line(aes(y = scaled_new_cases), color = "red") +
    geom_line(aes(y = pm2.5_avg), color = "blue") +
    ggtitle("Daily Covid Cases per 100,000") +
    xlab("Date") + ylab("Cases")
ggplotly(p)
```
...
```{r}
p <- covid_cases %>% filter(County == "Harris") %>% ggplot(aes(Date, cases_7day_avg)) + geom_line()
...
test_county = "Harris"
p_days <- days_high_pm %>% 
  filter(county == test_county, date >= "2020-03-01", date <= "2021-09-01") %>%
  ggplot(aes(date, n_high_pm)) + geom_line(color = "#5E747F") +
...
p_fatal_month <- monthly_covid_fatal %>% filter(County == test_county, date >= "2020-03-01", date <= "2021-09-01") %>% 
...
grid.arrange(p_days, p_fatal_month)
...
p2 <- daily_avg_pm2.5_county %>% filter(county == test_county, date_local >= "2020-03-01", date_local <= "2021-09-01") %>% 
...
test_county = "Harris"
p1 <- pm_7day %>% 
  filter(county == test_county, date_local >= "2020-03-01", date_local <= "2021-09-01") %>%
...
p1 <- pm_7day %>% 
  filter(county %in% test_county, date_local >= "2020-03-01", date_local <= "2021-09-01") %>%
...
covid_fatal %>% 
  group_by(County) %>% 
  # mutate(scaled_fatal_7day = rollmeanr(scaled_new_fatal, 7, fill = NA)) %>% ungroup() %>% 
  filter(County %in% test_county) %>%
  ggplot(aes(Date, new_fatal)) + geom_line(aes(color = County))
```
